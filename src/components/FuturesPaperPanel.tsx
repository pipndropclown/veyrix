...
export function FuturesPaperPanel({ portfolio, price, onOpen, onClose, autonomousEnabled, onAutonomousChange, strategyId, onStrategyChange }: {
  ...
  const position = (portfolio.futuresTrades ?? []).find((trade) => trade.status === "OPEN");
  ...
  return <section className="panel manual-panel futures-panel">
    ...
    <h2>Futures paper terminal · SOL / USDC</h2>
    <p>One shared virtual account. Margin is reserved from available USDC; exposure is simulated.</p>
    <label className="futures-auto-toggle"><input type="checkbox" checked={autonomousEnabled} onChange={(event) => onAutonomousChange(event.target.checked)} /> Autonomous futures signals (current chart timeframe, 10% of available virtual USDC)</label>
    ...
    <div className="futures-stats">
      <div><span>Total virtual equity</span><strong>{usd(portfolio.availableUsdc + portfolio.solBalance * (price ?? portfolio.averageSolEntryPrice ?? 0) + (position ? position.marginUsdc + Math.max(-position.marginUsdc, futuresPnl(position, price ?? position.entryPrice)) : 0))}</strong></div>
      <div><span>Available virtual collateral</span><strong>{usd(portfolio.availableUsdc)}</strong></div>
      <div><span>Margin in use</span><strong>{usd(position?.marginUsdc ?? 0)}</strong></div>
      <div><span>Futures exposure</span><strong>{usd(position?.exposureUsdc ?? 0)}</strong></div>
    </div>
    {position ? <div className="futures-position">
      <h3>Open {position.side} · {position.leverage}x · {position.source}</h3>
      <p>Entry {usd(position.entryPrice)} · Current {price ? usd(price) : "Unavailable"} · Liquidation {usd(position.liquidationPrice)}</p>
    ...
    <div className="table-wrap"><table><thead><tr><th>Side</th><th>Source</th><th>Entry</th><th>Exit</th><th>Margin</th><th>Exposure</th><th>P&amp;L</th><th>Status / reason</th></tr></thead><tbody>
      {(portfolio.futuresTrades ?? []).length ? (portfolio.futuresTrades ?? []).map((trade: FuturesTrade) => <tr key={trade.id}><td>{trade.side} {trade.leverage}x</td><td>{trade.source}</td><td>{usd(trade.entryPrice)}</td><td>{trade.exitPrice ? usd(trade.exitPrice) : "—"}</td><td>{usd(trade.marginUsdc)}</td><td>{usd(trade.exposureUsdc)}</td><td>{trade.realizedPnl === null ? "—" : usd(trade.realizedPnl)}</td><td>{trade.status} {trade.exitReason ?? ""}</td></tr>) : <tr><td colSpan={8}>No simulated futures trades yet.</td></tr>}
    </tbody></table></div>
  </section>;
}
