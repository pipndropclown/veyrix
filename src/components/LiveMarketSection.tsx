...
export function LiveMarketSection() {
  ...
  useEffect(()=>{
    ...
    const onStorage=(event:StorageEvent)=>{if(event.key!==PAPER_PORTFOLIO_STORAGE_KEY||!event.newValue)return;try{const restored=restorePaperLab({portfolioValue:JSON.parse(event.newValue)});if(restored.error){setAgentError(restored.error);return;}labRef.current=restored.state;setPortfolio(restored.state.portfolio);setAgentStore(restored.state.agents);}catch{setAgentError("Paper account changed in another tab but could not be restored.")}};
