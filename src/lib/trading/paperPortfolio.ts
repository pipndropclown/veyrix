...
export function restorePaperPortfolio(value: unknown, currentDay = localTradingDay(new Date())): PaperPortfolioState {
  ...
  for (const trade of multiMarketSpotTrades.filter(t=>t.status==="OPEN")) {
    ...
    spotPositions[trade.marketId]={marketId:trade.marketId,quantity:trade.quantity,averageEntryPrice:trade.entryPrice,entryTimestamp:trade.entryTimestamp,tradeId:trade.id,...(trade.agentId?{agentId:trade.agentId,agentName:trade.agentName}:{}),...(trade.agentDetached?{agentDetached:true}:{})};
  }
