...
export const FUTURES_FEE_RATE = 0.0006;
...
export function openFutures(portfolio: PaperPortfolioState, input: {
  ...
  const marketId=input.marketId??"SOL";
  if ((portfolio.futuresTrades ?? []).some((trade) => trade.status === "OPEN" && (trade.marketId??"SOL")===marketId))
    return { portfolio, error: `Close the open ${marketId} simulated futures position first.` };
  ...
  const trade: FuturesTrade = {
    ...
    source: input.source ?? "MANUAL", status: "OPEN", exitPrice: null, exitTimestamp: null, feesUsdc: input.marketId ? 0 : undefined,
...
export function closeFuturesMarket(portfolio: PaperPortfolioState, marketId:MarketId, price: number, timestamp = new Date().toISOString(), manual = false): PaperPortfolioState {
  portfolio = rollDay(portfolio, timestamp);
  const trade = (portfolio.futuresTrades ?? []).find((item) => item.status === "OPEN" && (item.marketId??"SOL")===marketId);
  ...
  const fees=liquidated||trade.feesUsdc===undefined?0:trade.exposureUsdc*FUTURES_FEE_RATE;
  const pnl = liquidated ? -trade.marginUsdc : Math.max(-trade.marginUsdc, futuresPnl(trade, price)-fees);
  ...
  return { ...portfolio, availableUsdc: portfolio.availableUsdc + trade.marginUsdc + pnl,
    ...
    futuresTrades: (portfolio.futuresTrades ?? []).map((item) => item.id === trade.id ?
      { ...item, status: "CLOSED" as const, exitPrice: price, exitTimestamp: timestamp, exitReason, realizedPnl: pnl,feesUsdc:fees } : item) };
}
export const closeFutures=(portfolio:PaperPortfolioState,price:number,timestamp=new Date().toISOString(),manual=false)=>closeFuturesMarket(portfolio,"SOL",price,timestamp,manual);
